...
<?php if(isset($bulan)) { ?>
 ...
 <?php $form = ActiveForm::begin(['fieldConfig' => [
   ...
   <table class="table table-hover table-bordered">
		...
		<tbody>
		<?php
			$sql = "select a.*, nama_basecamp, no_polisi, nama_proyek from ritasi_aspal a join basecamp b on a.asal = b.id join proyek c on c.id = a.tujuan join kendaraan_ritasi_aspal d on d.id_kendaraan = a.id_kendaraan where month(a.tanggal) = $bulan order by a.created_date desc";
			$models = RitasiAspal::findBySql($sql)->all();
		...
		?>
				<tr >
					...
					<td>
					<input class="form-control" type="text" name="aspal" value='<?php echo ($model['aspal']);?>'/>
